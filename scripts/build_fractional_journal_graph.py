...
from opensci_v2.transform import build_fractional_journal_edges


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--works", required=True, help="Works Parquet path")
    parser.add_argument("--output", required=True, help="Fractional journal edge Parquet path")
...
def main() -> None:
    ...
    edges_df = build_fractional_journal_edges(works_df)
    ...
    print(f"Wrote {len(edges_df)} fractional journal edges to {args.output}")
